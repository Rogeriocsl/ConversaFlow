...
- Um nó `collect` salva a resposta em `field` e segue para `target`.
...
Sessões, dados coletados e um histórico limitado são gravados atomicamente em `conversations.json`, dentro da pasta `userData` do Electron. A camada `ConversationPersistenceService` isola esse armazenamento para permitir uma futura migração para SQLite.
